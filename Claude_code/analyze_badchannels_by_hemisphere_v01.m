%% analyze_badchannels_by_hemisphere.m
%
% QUESTION
...
function analyze_badchannels_by_hemisphere(protocolfolder, outPrefix)
...
for s = 1:size(roster,1)
    ...
    stimlat = '';
    if isfield(ev,'sessioninfo') && isfield(ev.sessioninfo,'stimlat')
        stimlat = upper(strtrim(char(string(ev.sessioninfo.stimlat))));
    end
    if isempty(stimlat)
        warning('%s: stimlat unavailable; skipping.', subject); continue
    end
