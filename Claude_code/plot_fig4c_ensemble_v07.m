...
fprintf('\n=== Per-subject, Move phase ===\n');
for c = 1:size(SHOW_CELLS,1)
    fprintf('%s %s:\n', SHOW_CELLS{c,1}, SHOW_CELLS{c,2});
    for i = 1:numel(ids{c})
        fprintf('  %-6s BL %.4f  ES %.4f  LS %.4f  Post %.4f  (Post-BL %+.4f)\n', ...
            ids{c}{i}, V{c,1}(i,3,1), V{c,1}(i,3,2), V{c,1}(i,3,3), ...
            V{c,1}(i,3,4), V{c,1}(i,3,4)-V{c,1}(i,3,1));
    end
end
