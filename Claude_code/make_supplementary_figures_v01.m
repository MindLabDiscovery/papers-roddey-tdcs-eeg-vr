%% make_supplementary_figures_v01.m
%
...
sgt(sprintf(['Supp Fig 1A. Spectral power %g-%g Hz, 9 homologous pairs ' ...
    '(C3/C4 bold); midline excluded'], BAND_1A(1), BAND_1A(2)));
...
if ~isfile(COH_PATH)
    ...
    fh = figure('Position',[40 40 1180 380],'Visible','off');
    for p = 1:3
        ...
        for cc = 1:2
            ...
            for k = 1:numel(subs)
                for t = 1:4
                    ...
                    if any(m); V(k,t) = C.(bcol)(find(m,1)); end
                end
            end
            col = tern(cc==1,CRED,CGRY);
            for k = 1:size(V,1)
                plot(1:4, V(k,:), '-', 'Color',[col 0.28],'HandleVisibility','off');
            end
            errorbar(1:4, mean(V,1,'omitnan'), arrayfun(@(t)sem_(V(:,t)),1:4), ...
    ...
    sgt(sprintf(['Supp Fig 1D. Ensemble interhemispheric coherence, %s, ' ...
        'CS active vs sham   |   n=5, * p<.05, + p<.10'], COH_BAND));
...
print(fh, path, '-dpng','-r150'); close(fh);
fprintf('  wrote %s\n', path);
end
