function inspect_processingdata_v02(matfile)
...
% Empty struct arrays return nothing on field access, which errors on
% assignment. Report and stop.
if isempty(v)
    fprintf('%s  [EMPTY struct %s]\n', path, mat2str(size(v)));
    return
end

if numel(v) > 1
    fprintf('%s  [struct array %s]\n', path, mat2str(size(v)));
    % check every element for processingData, not just the first
    for k = 1:numel(v)
        if isfield(v(k),'processingData') && ~isempty(v(k).processingData)
            fprintf('%s(%d) has processingData\n', path, k);
        end
    end
    v = v(1);
    path = [path '(1)'];
