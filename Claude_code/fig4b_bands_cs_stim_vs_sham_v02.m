%% fig4b_bands_cs_stim_vs_sham_v02.m
%
% PURPOSE
...
%
% TIMEPOINTS
%   BL, ES, LS, Post - all four recording blocks, absolute values.
%   Every column is the same kind of quantity, so panels are directly
%   comparable. The early-versus-late contrast is introduced separately in
%   the figure that follows, where it is the point rather than one column
%   among several.
%
...
%
% OUTPUT
%   fig4b_bands.png            12 panels: 3 phases x 4 blocks
...
% FDR within the plotted measure (5 bands x 3 phases x 4 blocks = 60 tests)
...
nb = size(bands,1);
fh = figure('Position',[30 40 1500 780],'Visible','off');
sp = 0;
...
for p = 1:3
    for t = 1:4
        ...
        if strcmp(MEASURE,'diff'); plot([0.4 nb+0.6],[0 0],'k:','HandleVisibility','off'); end
...
%% ==================== LOCAL FUNCTIONS =========================
function Y = tps(X)
% All four recording blocks, absolute values, passed through unchanged.
% X: subj x band x phase x block  ->  Y: same
% Earlier versions collapsed to pre / LS-ES / post. That is now handled by
% the separate LS-ES figure that follows, so this panel shows the raw time
% course and every column is an absolute value on a common footing.
Y = X;
end
