...
for i = 1:nS
    ...
    psd_bl = mean(get_spec(sd, mi, ch_idx, PRIMARY_PHASE, TRIAL_BL, IS_DB), 2, 'omitnan');
    psd_es = mean(get_spec(sd, mi, ch_idx, PRIMARY_PHASE, TRIAL_ES, IS_DB), 2, 'omitnan');
    psd_ls = mean(get_spec(sd, mi, ch_idx, PRIMARY_PHASE, TRIAL_LS, IS_DB), 2, 'omitnan');
    ...
    for ph = 1:numel(PHASES)
        pe = mean(get_spec(sd, mi, ch_idx, ph, TRIAL_ES, IS_DB), 2, 'omitnan');
        pl = mean(get_spec(sd, mi, ch_idx, ph, TRIAL_LS, IS_DB), 2, 'omitnan');
...
for c = 1:size(cells,1)
    ...
    for k = idx
        ...
        e = get_spec(sd, mi, ch, PRIMARY_PHASE, TRIAL_ES, IS_DB);
        l = get_spec(sd, mi, ch, PRIMARY_PHASE, TRIAL_LS, IS_DB);
...
%% ================= LOCAL FUNCTIONS ==============================
function sg = get_spec(sd, sidx, ch_idx, phase, trial, is_db)
% Spectrogram [nFreq x nTime] for one trial & phase at one channel, in dB.
% sd and is_db are passed explicitly: local functions in a script do not
