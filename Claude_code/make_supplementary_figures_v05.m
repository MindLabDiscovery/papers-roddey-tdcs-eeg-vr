%% make_supplementary_figures_v05.m
%
...
tpLbl1A  = {'BL','LS-ES','Post'};   % timepoints shown in Supp Fig 1A
...
for c = 1:size(cells,1)
    ...
    for p = PSD_PHASES
        for side = 1:2
            ...
            G4 = squeeze(mean(A,1,'omitnan'));     % pair x block (BL ES LS Post)
            % Collapse to three timepoints: baseline, the early-versus-late
            % stimulation contrast, and post-stimulation. The middle point is
            % a DIFFERENCE while the outer two are ABSOLUTE power, so it sits
            % near zero by construction; the dip is arithmetic, not a finding.
            % Using LS-ES rather than ES and LS separately removes the
            % stationary stimulation artifact, which is present in both.
            G = [G4(:,1), G4(:,3)-G4(:,2), G4(:,4)];
            for ee = 1:size(G,1)
                plot(1:3, G(ee,:), 'Color',[0.72 0.72 0.72]);
            end
            plot(1:3, G(IDX_C3C4,:), 'LineWidth',2, ...
                'Color', tern(side==1,[0.15 0.30 0.75],[0.75 0.30 0.15]));
            plot([0.8 3.2],[0 0],'k:','HandleVisibility','off');
            set(gca,'XTick',1:3,'XTickLabel',tpLbl1A,'FontSize',7, ...
                'XTickLabelRotation',20);
            xlim([0.8 3.2]); grid on;
...
sgt(sprintf(['Supp Fig 1A. Spectral power %g-%g Hz, 9 homologous pairs (C3/C4 bold); ' ...
    'midline excluded; middle point is a difference; common y-axis'], ...
    BAND_1A(1), BAND_1A(2)));
