...
%% ============================ CONFIG ============================
COH_PATH = fullfile(pwd,'coh_multiband_cells.csv');   % recompute_coherence_multiband.m
% Band to plot. The multiband CSV carries one ens_<band> column per band,
% so the band can be switched here WITHOUT re-running FieldTrip.
%   'gamma2545' 25-45 Hz  - recommended: stronger at Move/Post (p=.006 vs
%                           .014), clear of the 50 Hz ceiling so the
%                           edge-artifact objection does not apply, and
%                           matches the band where the ES-LS spectral
%                           residual was largest.
%   'gamma'     30-50 Hz  - as originally computed, as in the 4B caption.
%   also: 'delta','theta','alpha','beta'
% Set to '' to use the absimag_ensemble column as written.
COH_BAND = 'gamma2545';
...
%% ============================ LOAD ==============================
if ~isfile(COH_PATH)
    error('%s not found. Run recompute_coherence_multiband.m first.', COH_PATH);
end
...
% Select the band from the multiband columns, if present.
if ~isempty(COH_BAND)
    bcol = ['ens_' COH_BAND];
    if ismember(bcol, C.Properties.VariableNames)
        C.absimag_ensemble = C.(bcol);
        pcol = ['pt_' COH_BAND];
        if ismember(pcol, C.Properties.VariableNames)
            C.absimag_pertrial = C.(pcol);
        end
        fprintf('Band: %s (column %s)\n', COH_BAND, bcol);
    else
        warning(['Column %s not found; using absimag_ensemble as written. ' ...
                 'Is this the multiband CSV?'], bcol);
    end
end
...
for ph = 1:3
    ...
    if size(SHOW_CELLS,1) == 2
        pv3 = nan(1,3);
        [~,pv3(1)] = ttest2_safe(V{1,1}(:,ph,1),           V{2,1}(:,ph,1));
        [~,pv3(2)] = ttest2_safe(V{1,1}(:,ph,3)-V{1,1}(:,ph,2), ...
                                 V{2,1}(:,ph,3)-V{2,1}(:,ph,2));
        [~,pv3(3)] = ttest2_safe(V{1,1}(:,ph,4),           V{2,1}(:,ph,4));
        add_sig(1:3, pv3);
    end
...
print(fh,fullfile(OUT_DIR,sprintf('fig4c_ensemble_3point_%s.png',tern(isempty(COH_BAND),'aswritten',COH_BAND))),'-dpng','-r150'); close(fh);
...
for ph = 1:3
    ...
    plot([0.6 3.4],[0 0],'k:','HandleVisibility','off');
    if size(SHOW_CELLS,1) == 2
        pvd = nan(1,3); pairs = [2 1; 3 2; 4 3];
        for k = 1:3
            [~,pvd(k)] = ttest2_safe( ...
                V{1,1}(:,ph,pairs(k,1))-V{1,1}(:,ph,pairs(k,2)), ...
                V{2,1}(:,ph,pairs(k,1))-V{2,1}(:,ph,pairs(k,2)));
        end
        add_sig(1:3, pvd);
    end
...
sgtitle_compat(sprintf('Successive change across the session, corrected ensemble, %s band', tern(isempty(COH_BAND),'as-written',COH_BAND)));
print(fh,fullfile(OUT_DIR,sprintf('fig4c_ensemble_deltas_%s.png',tern(isempty(COH_BAND),'aswritten',COH_BAND))),'-dpng','-r150'); close(fh);
...
for ph = 1:3
    ...
    % significance markers vs the second plotted group, per block
    if size(SHOW_CELLS,1) == 2
        pv4 = nan(1,4);
        for b = 1:4
            aa = V{1,1}(:,ph,b); cc = V{2,1}(:,ph,b);
            [~,pv4(b)] = ttest2_safe(aa,cc);
        end
        add_sig(1:4, pv4);
    end
...
sgtitle_compat(sprintf('Figure 4C: corrected vs published estimator, %s band', tern(isempty(COH_BAND),'as-written',COH_BAND)));
print(fh,fullfile(OUT_DIR,sprintf('fig4c_ensemble_absolute_%s.png',tern(isempty(COH_BAND),'aswritten',COH_BAND))),'-dpng','-r150'); close(fh);
...
writetable(cell2table(rows,'VariableNames', ...
    ...
    fullfile(OUT_DIR,sprintf('fig4c_ensemble_values_%s.csv',tern(isempty(COH_BAND),'aswritten',COH_BAND))));

fprintf('\nWrote %s\n', OUT_DIR);
fprintf('fig4c_ensemble_absolute.png is the one to use for the manuscript.\n');

%% ==================== LOCAL FUNCTIONS =========================
function add_sig(xs, pvals)
% Place significance markers at the top of the current axes.
% ASCII only and Interpreter set to 'none': MATLAB's TeX interpreter does
% not support \dagger, which errors rather than rendering.
%   *  p < 0.05      +  p < 0.10
yl = ylim;
span = yl(2) - yl(1);
ypos = yl(2) - 0.04*span;
for k = 1:numel(xs)
    ...
    text(xs(k), ypos, mk, 'HorizontalAlignment','center', ...
        'FontSize',fs, 'FontWeight','bold', 'Interpreter','none', ...
        'Clipping','off');
end
ylim(yl);   % text() can expand limits; restore
end
