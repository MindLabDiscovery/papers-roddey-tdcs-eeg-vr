...
%
% OUTPUTS
%   fig_drift_psd.png     PSD by block, log-log, per group, drift shaded
%   fig_drift_bands.png   band power by block, drift vs other bands
%   fig_drift_subjects.png per-subject drift-band trajectory
%   drift_values.csv
...
for c = 1:size(cells,1)
    ...
    shade(bands{1,2},[0.95 0.85 0.85]);
    ...
    set(gca,'XScale','log'); shade(bands{1,2},[0.95 0.85 0.85]);
    grid on; xlim([max(min(f),0.5) max(f)]);
    xlabel('Hz (log)');
    if c==1; ylabel('power (dB)'); end
end
sgt('PSD at the anodal electrode by block; drift band (1-4 Hz) shaded');
print(fh,fullfile(OUT_DIR,'fig_drift_psd.png'),'-dpng','-r150'); close(fh);
...
sgt('Change from baseline by band: drift versus the rest of the spectrum');
print(fh,fullfile(OUT_DIR,'fig_drift_bands.png'),'-dpng','-r150'); close(fh);
...
sgt('Drift-band (1-4 Hz) power per subject across blocks');
print(fh,fullfile(OUT_DIR,'fig_drift_subjects.png'),'-dpng','-r150'); close(fh);
