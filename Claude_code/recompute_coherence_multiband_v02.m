function recompute_coherence_multiband_v02(protocolfolder, outPrefix, ftPath)
...
SAVE_SPECTRA = true;          % write the FULL coherence spectrum per cell,
                              % enabling a coherogram (coherence vs frequency)
                              % rather than band averages alone
SPEC_FMAX    = 100;           % upper limit written out, Hz
...
for s = 1:size(roster,1)
    ...
    for b = 1:numel(blockNames)
        ...
        for p = 1:min(3,size(wk,1))
            ...
            try
                [ensV, ptV, nTr, c3i, c4i, K, specF, specV] = ...
            ...
            if SAVE_SPECTRA && ~isempty(specF)
                keep = specF <= SPEC_FMAX;
                for q = find(keep)
                    specRows(end+1,:) = {subject, grp, cond, ...
                        blockLabels{b}, phaseNames{p}, specF(q), specV(q)}; %#ok<AGROW>
                end
            end
        end
    end
...
if SAVE_SPECTRA && ~isempty(specRows)
    SP = cell2table(specRows,'VariableNames', ...
        {'subject','group','condition','block','phase','freq','icoh'});
    specCsv = [outPrefix '_spectra.csv'];
    writetable(SP, specCsv);
    fprintf('Full coherence spectra written: %s (%d rows, %d freq bins to %g Hz)\n', ...
        specCsv, height(SP), numel(unique(SP.freq)), SPEC_FMAX);
    fprintf('  Use plot_coherogram.m to render coherence vs frequency.\n');
end
fprintf('\n');
...
% =========================================================================
function [ensV, ptV, nTrials, c3interp, c4interp, K, specF, specV] = ...
...
% full spectrum, for the coherogram
specF = connE.freq(:)';
specV = abs(imag(connE.cohspctrm(ip,:)));
specV = specV(:)';
